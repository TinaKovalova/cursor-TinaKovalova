...
function getPairs(arrStudents) {
    const pairs = [];
    const women = [];
    arrStudents.forEach(item => item.endsWith('а') ? women.push(item) : pairs.push([item]));
    pairs.forEach((pair, index) => pair.push(women[index]));
    return pairs;
}

function addThemesToPair(arrThemes, arrPairs) {
    return arrPairs.map((pair, index) => [pair.join(' і '), arrThemes[index]]);
}
